...
exports.getLocationBySlug = async (req, res) => {
    try {
        ...
        const [jobCount, companyCount, recentJobs, locationCompanies] = await Promise.all([
            Job.count({ where: { locationId: location.id } }),
            Company.count({ where: { locationId: location.id } }),
            Job.findAll({
        ...
        const parent = location.parentId ? await Location.findByPk(location.parentId) : null;
