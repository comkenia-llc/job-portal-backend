...
exports.getAdminStats = async (req, res) => {
    try {
        ...
        const [users, companies, jobs, locations] = await Promise.all([
            User.count(),
            Company.count({ where: applyMarketScope({}, req, { allowAdminOverride: true, allowAllForAdmin: true }) }),
            Job.count({ where: applyMarketScope({}, req, { allowAdminOverride: true, allowAllForAdmin: true }) }),
            Location.count({ where: applyMarketScope({}, req, { allowAdminOverride: true, allowAllForAdmin: true }) }),
        ]);
...
exports.getAnalytics = async (req, res) => {
    try {
        ...
        const topCompanies = await Job.findAll({
            ...
            where: applyMarketScope({}, req, { allowAdminOverride: true, allowAllForAdmin: true }),
