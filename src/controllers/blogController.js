...
exports.createPost = async (req, res) => {
    try {
        ...
        const payload = assignMarketToPayload({ ...req.body }, req, { allowAdminOverride: true });
        const { title, content, excerpt, coverImage, status, faqs, market } = payload;
...
exports.updatePost = async (req, res) => {
    try {
        ...
        if (req.user.role === "employer") {
            ...
            if (post.market && access.market && post.market !== access.market) {
                return res.status(403).json({ error: "You can only manage posts for your assigned market" });
            }
        }
        ...
        const payload = assignMarketToPayload({ ...req.body }, req, { allowAdminOverride: true });
        const { title, content, excerpt, coverImage, status, removeCover, faqs, market } = payload;

        if (title && title.trim() !== post.title) {
            post.slug = await generateUniqueSlug(title, post.id, market || post.market);
...
exports.listMyPosts = async (req, res) => {
    try {
        ...
        const where =
            req.user.role === "admin"
                ? applyMarketScope({}, req, { allowAdminOverride: true, allowAllForAdmin: true })
                : applyMarketScope({ authorId: req.user.id }, req, {
                      allowAdminOverride: true,
                      allowAllForAdmin: true,
                  });
...
exports.getPostBySlug = async (req, res) => {
    try {
        const post = await BlogPost.findOne({
            where: applyMarketScope({ slug: req.params.slug, status: "published" }, req, {
                allowAdminOverride: true,
                allowAllForAdmin: true,
            }),
            include: [{ model: User, as: "author", attributes: ["id", "username", "firstName", "lastName"] }],
