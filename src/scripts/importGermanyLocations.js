...
/**
 * Import Germany location hierarchy from src/data/germany-hierarchy.json
 * into the Locations table with Germany market scoping and SEO metadata.
 *
 * Usage: node src/scripts/importGermanyLocations.js
 */
...
const HIERARCHY_PATH = path.resolve(
    __dirname,
    "../data/germany-hierarchy.json"
);
const MARKET = "germany";
...
const CURRENCY = "EUR";
const IMPORT_IMAGES =
    String(process.env.IMPORT_GERMANY_LOCATION_IMAGES || "false")
        .trim()
        .toLowerCase() === "true";
const FLAG_PATH =
    process.env.GERMANY_FLAG_PATH || "/uploads/locations/flags/default.png";
const DEFAULT_IMAGE = process.env.GERMANY_DEFAULT_LOCATION_IMAGE || null;
const PUBLIC_BASE_URL = (
    process.env.GERMANY_PUBLIC_APP_URL ||
    process.env.PUBLIC_APP_URL ||
    "https://germanyjobzone.com"
)
    .trim()
    .replace(/\/+$/, "");
...
const loadHierarchy = () => {
    if (!fs.existsSync(HIERARCHY_PATH)) {
        throw new Error(`Germany hierarchy file not found at ${HIERARCHY_PATH}`);
    }
...
const run = async () => {
    ...
    console.log(
        `✅ Germany locations import complete. Created: ${stats.created}, Updated: ${stats.updated}`
    );
};

run()
    .catch((err) => {
        console.error("❌ Germany locations import failed:", err);
        process.exitCode = 1;
    })
    .finally(async () => {
        try {
