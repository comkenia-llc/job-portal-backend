...
/**
 * Import Australia location hierarchy from src/data/australia-hierarchy.json
 * into the Locations table with Australia market scoping and SEO metadata.
 *
 * Usage: node src/scripts/importAustraliaLocations.js
 */
...
const CURRENCY = "AUD";
const IMPORT_IMAGES =
    String(process.env.IMPORT_AUSTRALIA_LOCATION_IMAGES || "false")
        .trim()
        .toLowerCase() === "true";
const FLAG_PATH =
    process.env.AUSTRALIA_FLAG_PATH || "/uploads/locations/flags/default.png";
const DEFAULT_IMAGE = process.env.AUSTRALIA_DEFAULT_LOCATION_IMAGE || null;
const PUBLIC_BASE_URL = (
    process.env.AUSTRALIA_PUBLIC_APP_URL ||
    process.env.PUBLIC_APP_URL ||
    "https://australiajobzone.com"
)
    .trim()
    .replace(/\/+$/, "");
...
const loadHierarchy = () => {
    if (!fs.existsSync(HIERARCHY_PATH)) {
        throw new Error(`Australia hierarchy file not found at ${HIERARCHY_PATH}`);
    }
...
const run = async () => {
    ...
    console.log(
        `✅ Australia locations import complete. Created: ${stats.created}, Updated: ${stats.updated}`
    );
};

run()
    .catch((err) => {
        console.error("❌ Australia locations import failed:", err);
        process.exitCode = 1;
    })
    .finally(async () => {
        try {
