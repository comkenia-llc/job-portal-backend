...
/**
 * Import Saudi Arabia location hierarchy from src/data/saudi-hierarchy.json
 * into the Locations table with Saudi market scoping and SEO metadata.
 *
 * Usage: node src/scripts/importSaudiLocations.js
 */
...
const HIERARCHY_PATH = path.resolve(__dirname, "../data/saudi-hierarchy.json");
const MARKET = "saudi";
...
const CURRENCY = "SAR";
const IMPORT_IMAGES =
    String(process.env.IMPORT_SAUDI_LOCATION_IMAGES || "false").trim().toLowerCase() ===
    "true";
const FLAG_PATH =
    process.env.SAUDI_FLAG_PATH || "/uploads/locations/flags/default.png";
const DEFAULT_IMAGE = process.env.SAUDI_DEFAULT_LOCATION_IMAGE || null;
const PUBLIC_BASE_URL = (
    process.env.SAUDI_PUBLIC_APP_URL ||
    process.env.PUBLIC_APP_URL ||
    "https://saudijobzone.com"
)
    .trim()
    .replace(/\/+$/, "");
...
const loadHierarchy = () => {
    if (!fs.existsSync(HIERARCHY_PATH)) {
        throw new Error(`Saudi hierarchy file not found at ${HIERARCHY_PATH}`);
    }
...
const run = async () => {
    ...
    console.log(
        `✅ Saudi locations import complete. Created: ${stats.created}, Updated: ${stats.updated}`
    );
};

run()
    .catch((err) => {
        console.error("❌ Saudi locations import failed:", err);
        process.exitCode = 1;
    })
    .finally(async () => {
        try {
