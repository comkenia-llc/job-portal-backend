...
/**
 * Import Pakistan location hierarchy from src/data/pk-hierarchy.json
 * into the Locations table with Pakistan market scoping and SEO metadata.
 *
 * Usage: node src/scripts/importPkLocations.js
 */
...
const HIERARCHY_PATH = path.resolve(__dirname, "../data/pk-hierarchy.json");
const MARKET = "pk";
...
const CURRENCY = "PKR";
const IMPORT_IMAGES =
    String(process.env.IMPORT_PK_LOCATION_IMAGES || "false").trim().toLowerCase() ===
    "true";
const FLAG_PATH =
    process.env.PK_FLAG_PATH || "/uploads/locations/flags/default.png";
const DEFAULT_IMAGE = process.env.PK_DEFAULT_LOCATION_IMAGE || null;
...
const loadHierarchy = () => {
    if (!fs.existsSync(HIERARCHY_PATH)) {
        throw new Error(`Pakistan hierarchy file not found at ${HIERARCHY_PATH}`);
    }
...
const run = async () => {
    ...
    console.log(
        `✅ Pakistan locations import complete. Created: ${stats.created}, Updated: ${stats.updated}`
    );
};

run()
    .catch((err) => {
        console.error("❌ Pakistan locations import failed:", err);
        process.exitCode = 1;
    })
    .finally(async () => {
        try {
