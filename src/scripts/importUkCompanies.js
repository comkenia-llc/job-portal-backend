...
/**
 * UK Companies Importer
 * - Wikidata-based seed import
 * - UK market scoping
 * - Location mapping against imported UK locations
 * - SEO-ready company fields for /employers/{slug}
 *
 * Usage:
 *   node src/scripts/importUkCompanies.js
 */
...
const PAGE_SIZE = parseInt(process.env.UK_COMPANY_IMPORT_PAGE_SIZE || "500", 10);
const MAX_PAGES = parseInt(process.env.UK_COMPANY_IMPORT_MAX_PAGES || "20", 10);
const DELAY_MS = parseInt(process.env.UK_COMPANY_IMPORT_DELAY_MS || "1200", 10);
const CREATED_BY = parseInt(process.env.UK_COMPANY_IMPORT_CREATED_BY || "1", 10);
...
function buildAbout({ name, industry, locationName, foundedYear }) {
    const clauses = [
        `${name} is an employer profile focused on UK hiring demand${locationName ? ` across ${locationName}` : ""}.`,
...
function buildCompanySeo({ name, industry, locationName, slug }) {
    ...
    const place = locationName || "the UK";
    const sector = industry || "business";

    const titlePatterns = [
        `${name} Jobs in ${place} | ${sector} Careers UK`,
        `${name} Careers UK | Hiring in ${place}`,
        `${name} Employer Profile | ${sector} Jobs in ${place}`,
        `${name} Vacancies in ${place} | UK Careers`,
    ...
    const descOpeners = [
        ...
        `Browse ${name} company insights and UK vacancies linked to ${place}.`,
    ];

    const descClosers = [
        `Compare roles, employer context, and search-ready company information for ${sector} candidates.`,
        `Use this employer page to evaluate active demand, company positioning, and UK career relevance.`,
        `This profile is structured to connect candidates with searchable company information and location-linked opportunities.`,
        `Find company context, sector signals, and discoverability tailored to UK employer search intent.`,
...
async function run() {
    console.log("🚀 Importing UK companies with SEO metadata...");
    ...
    for (let page = 0; page < MAX_PAGES; page++) {
        ...
        console.log(`📦 UK companies page ${page + 1} (OFFSET ${offset})`);

        const res = await fetch(
            `${ENDPOINT}?format=json&query=${encodeURIComponent(query)}`,
            {
                headers: {
                    "User-Agent": "UKCompanyImporter/1.0 (jobs platform)",
        ...
        if (!rows.length) {
            console.log("🛑 No more UK company data");
            break;
        }
        ...
        for (const row of rows) {
            try {
                ...
                const payload = {
                    ...
                    foundedYear,
                    tagline: industry
                        ? `${name} careers in ${locationName || "the UK"} across ${industry}.`
                        : `${name} careers and employer profile for UK job seekers.`,
                    about: buildAbout({
                        name,
                        industry,
                        locationName,
                ...
                skippedCount++;
                console.warn("⚠️ Skipped UK company row:", err.message);
            }
        }
    ...
    console.log("🎉 UK company import complete");
    console.log({ createdCount, updatedCount, skippedCount });
}

run().catch((err) => {
    console.error("❌ UK company import failed:", err.message);
    process.exit(1);
});
