...
const { JobCategory, sequelize } = require('../models');

const repoRoot = path.resolve(__dirname, '..', '..', '..');
const defaultPath = path.join(repoRoot, 'data', 'job-category-faqs.json');
...
function normalizePayload(data) {
    ...
    if (data && Array.isArray(data.jobCategories)) return data.jobCategories;
    return [];
}
...
async function run() {
    ...
    console.log('📊 JobCategories count:', totalCategories);
    if (!totalCategories) {
        throw new Error(
            `No JobCategories found in DB "${dbName}". Check DB_NAME/DB_USER in back/.env or pass --db-name/--db-user.`
        );
    }
    ...
    if (!items.length) {
        throw new Error('No job categories found in JSON.');
    }
