...
/**
 * Import US location hierarchy from src/data/us-hierarchy.json
 * into the Locations table with USA market scoping and SEO metadata.
 *
 * Usage: node src/scripts/importUsLocations.js
 */
...
const HIERARCHY_PATH = path.resolve(__dirname, "../data/us-hierarchy.json");
const MARKET = "usa";
...
const CURRENCY = "USD";
const IMPORT_IMAGES =
    String(process.env.IMPORT_US_LOCATION_IMAGES || "false")
        .trim()
        .toLowerCase() === "true";
const FLAG_PATH =
    process.env.US_FLAG_PATH || "/uploads/locations/flags/default.png";
const DEFAULT_IMAGE = process.env.US_DEFAULT_LOCATION_IMAGE || null;
const PUBLIC_BASE_URL = (
    process.env.USA_PUBLIC_APP_URL ||
    process.env.PUBLIC_APP_URL ||
    "https://usajobzone.com"
)
    .trim()
    .replace(/\/+$/, "");
...
const loadHierarchy = () => {
    if (!fs.existsSync(HIERARCHY_PATH)) {
        throw new Error(`US hierarchy file not found at ${HIERARCHY_PATH}`);
    }
...
const run = async () => {
    ...
    console.log(
        `✅ US locations import complete. Created: ${stats.created}, Updated: ${stats.updated}`
    );
};

run()
    .catch((err) => {
        console.error("❌ US locations import failed:", err);
        process.exitCode = 1;
    })
    .finally(async () => {
        try {
