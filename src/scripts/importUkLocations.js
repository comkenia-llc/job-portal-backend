...
/**
 * Import UK location hierarchy from src/data/uk-hierarchy.json
 * into the Locations table with UK market scoping and SEO metadata.
 *
 * Usage: node src/scripts/importUkLocations.js
 */
...
const HIERARCHY_PATH = path.resolve(__dirname, "../data/uk-hierarchy.json");
const MARKET = "uk";
...
const CURRENCY = "GBP";
const IMPORT_IMAGES =
    String(process.env.IMPORT_UK_LOCATION_IMAGES || "false").trim().toLowerCase() ===
    "true";
const FLAG_PATH =
    process.env.UK_FLAG_PATH || "/uploads/locations/flags/default.png";
const DEFAULT_IMAGE = process.env.UK_DEFAULT_LOCATION_IMAGE || null;
...
const loadHierarchy = () => {
    if (!fs.existsSync(HIERARCHY_PATH)) {
        throw new Error(`UK hierarchy file not found at ${HIERARCHY_PATH}`);
    }
...
const run = async () => {
    ...
    await sequelize.transaction(async (transaction) => {
        ...
        for (const state of data.states || []) {
            ...
            if (!registerSlug(statePayload.type, statePayload.slug)) {
                console.log(`⚠️ Duplicate state slug ${statePayload.slug} skipped`);
                continue;
            }
            ...
            for (const city of state.cities || []) {
                ...
                for (const neighborhood of city.neighborhoods || []) {
                    ...
                    if (
                        ...
                        console.log(
                            `⚠️ Duplicate neighborhood slug ${neighborhoodPayload.slug} skipped`
                        );
                        continue;
                    }
    ...
    console.log(
        `✅ UK locations import complete. Created: ${stats.created}, Updated: ${stats.updated}`
    );
};
...
run()
    .then(() => process.exit(0))
    .catch((err) => {
        console.error("❌ Failed to import UK locations:", err);
        process.exit(1);
    });
