...
/**
 * Build a hierarchical Australia locations JSON from the exported OSM GeoJSON sequence.
 * Usage: node src/scripts/buildAustraliaLocations.js
 */
...
const OUTPUT_PATH = path.resolve(
    __dirname,
    "../data/australia-hierarchy.json"
);

const STATE_DEFS = [
    { name: "New South Wales", aliases: ["new south wales", "nsw"] },
    { name: "Victoria", aliases: ["victoria", "vic"] },
    { name: "Queensland", aliases: ["queensland", "qld"] },
    { name: "Western Australia", aliases: ["western australia", "wa"] },
    { name: "South Australia", aliases: ["south australia", "sa"] },
    { name: "Tasmania", aliases: ["tasmania", "tas"] },
    { name: "Australian Capital Territory", aliases: ["australian capital territory", "act"] },
    { name: "Northern Territory", aliases: ["northern territory", "nt"] },
];
...
const buildHierarchy = async () => {
    ...
    for (const def of STATE_DEFS) {
        ...
        const centroid = geometry
            ...
            : { lat: null, lng: null };
        const slug = `australia-${slugify(def.name)}`;
        const stateData = {
            name: def.name,
            slug,
            lat: centroid.lat,
    ...
    console.log(
        `✅ Australia hierarchy written to ${path.relative(process.cwd(), OUTPUT_PATH)}`
    );
    console.log(
...
buildHierarchy().catch((err) => {
    console.error("❌ Failed to build Australia hierarchy:", err);
    process.exit(1);
});
