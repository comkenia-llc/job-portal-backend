...
const formatSalary = ({ salaryMin, salaryMax, currency = "AED" }) => {
    if (!salaryMin && !salaryMax) return "Not provided";
...
function adminNewJobTemplate({
    ...
    reviewUrl = "https://dubaijobzone.com/admin/jobs",
    jobPreviewUrl = "",
    adminDashboardUrl = "https://dubaijobzone.com/admin",
    siteUrl = "https://dubaijobzone.com",
}) {
    const salaryText = formatSalary({ salaryMin, salaryMax, currency });

    const safeJobTitle = escapeHtml(jobTitle);
    const safeJobSlug = escapeHtml(jobSlug);
    const safeJobType = escapeHtml(jobType);
    const safeJobStatus = escapeHtml(jobStatus);
    ...
    const safeExperienceLevel = escapeHtml(experienceLevel);
    const safeEducationLevel = escapeHtml(educationLevel);
    const safeDeadline = deadline ? escapeHtml(formatDate(deadline)) : "";
    const safeSubmittedAt = escapeHtml(formatDate(submittedAt));
    const safeReviewUrl = escapeHtml(reviewUrl);
    const safeJobPreviewUrl = escapeHtml(jobPreviewUrl);
    const safeAdminDashboardUrl = escapeHtml(adminDashboardUrl);
    const safeSiteUrl = escapeHtml(siteUrl);

    const subject = `New job submitted for review: ${jobTitle}`;

    const text = `
New job submitted on Dubai Job Zone.

Job title: ${jobTitle}
Job slug: ${jobSlug || "Not provided"}
Status: ${jobStatus}
Type: ${jobType || "Not provided"}
Company: ${companyName || "Not provided"}
Employer: ${employerName || "Not provided"}
...
Deadline: ${deadline ? formatDate(deadline) : "Not provided"}
Submitted at: ${formatDate(submittedAt)}

Review job:
${reviewUrl}

${jobPreviewUrl ? `Preview job:\n${jobPreviewUrl}` : ""}
...
<body style="margin:0;padding:0;background:#eef3f8;font-family:Arial,Helvetica,sans-serif;color:#0f172a;-webkit-font-smoothing:antialiased;">
  ...
  <div style="display:none;max-height:0;overflow:hidden;font-size:1px;line-height:1px;color:#eef3f8;opacity:0;">
    New job submitted for review: ${safeJobTitle}.
  </div>
                ...
                <tr>
                  <td style="vertical-align:middle;">
                    ...
                    <div style="margin-top:8px;font-size:14px;line-height:22px;color:#c7d2fe;">
                      Admin notification • New job submission
                    </div>
                  </td>
                </tr>
              </table>
          ...
          <tr>
            <td style="padding:38px 34px 30px;">
              ...
              <p style="margin:0 0 24px;font-size:16px;line-height:26px;color:#334155;">
                An employer has submitted a new job on Dubai Job Zone. Review the job details before publishing it to protect listing quality and user trust.
              </p>
                  ...
                  <td style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:22px;padding:22px 20px;">
                    ...
                    <table width="100%" cellpadding="0" cellspacing="0" role="presentation">
                      ...
                      <tr>
                        ...
                        <td style="padding:8px 0;font-size:14px;line-height:22px;color:#0f172a;">
                          ${safeExperienceLevel || "Not provided"}
                        </td>
                      </tr>
                      ...
                      <tr>
                        ...
                        <td style="padding:8px 0;font-size:14px;line-height:22px;color:#0f172a;">
                          ${safeEducationLevel || "Not provided"}
                        </td>
                      </tr>
                ...
                <tr>
                  <td style="background:#fff7ed;border:1px solid #fed7aa;border-radius:18px;padding:17px 18px;">
                    ...
                    <p style="margin:0;font-size:14px;line-height:22px;color:#9a3412;">
                      Check that the job title, salary, location, employer details, description quality, and application method look legitimate before publishing.
                    </p>
                  </td>
                </tr>
              </table>
              ...
              <table cellpadding="0" cellspacing="0" role="presentation" style="margin:0 0 28px;">
                <tr>
                  <td align="center" bgcolor="#f8fafc" style="border-radius:14px;border:1px solid #e2e8f0;">
                    <a href="${safeJobPreviewUrl}" target="_blank" style="display:inline-block;padding:13px 22px;font-size:14px;font-weight:900;color:#2563eb;text-decoration:none;border-radius:14px;background:#f8fafc;">
                      Preview Job
                    </a>
                  </td>
                </tr>
          ...
          <tr>
            <td style="padding:24px 34px 34px;background:#ffffff;">
              ...
              <p style="margin:0 0 14px;font-size:13px;line-height:21px;color:#64748b;">
                This email was generated automatically because a new job was submitted on Dubai Job Zone.
              </p>
